...
import gmpy2
from gmpy2 import mpfr, sin, cos, const_pi
from numpy import dot, diff, multiply
import pandas as pd
...
for bits in (32, 64):
    ...
    for s in (mp(10)**-ex for ex in range(1,20)):
    ...
    dxRes = results[['Approximation', 'Error']]
    print(dxRes)
    print('\nResults sorted by Squared Error:')
    errRes = results[['Approximation', 'Error/AnalyVal', 'SqError']].sort_values(by='SqError')
    print(errRes)
    print('------------------------------------------------')
    
    dfi.export(dxRes, 'trab1_results_{}_by_dx.png'.format(bits))
    dfi.export(errRes, 'trab1_results_{}_by_err.png'.format(bits))
